...
program
    ...
    try {
        ...
        await processBlocks(scene, blockIndices, palette, geometry, width, height, depth);
...
async function processBlocks(scene, blockIndices, palette, geometry, width, height, depth) {
    let addedBlocks = 0;
    const airIndex = palette.findIndex((block) => block.name?.value === "minecraft:air");
...
async function colorizeTexture(texturePath, hexColor) {
...
async function getBlockMaterial(blockName) {
    blockName = blockName.replace("minecraft:", "");
    // Map for Bedrock to Java block name differences
    const blockNameMap = {
        melon_block: "melon",
    };
    if (blockNameMap[blockName]) {
        blockName = blockNameMap[blockName];
    }
